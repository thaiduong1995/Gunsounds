    ...
    override fun load(context: Context, callback: NativeAdCallback?): CemNativeAdView {
        AdLoader.Builder(context, adUnitId ?: return this).forNativeAd { ads: NativeAd ->
            ...
            override fun onAdLoaded() {
                ...
                mNativeAd?.let {ads ->
                    ads.setOnPaidEventListener{
                        AppFlyersLogManager.logEventNativeAdRevenue(ads, adUnitId, it)
                    }
                }
            }
        }).withNativeAdOptions(
        ...
        try {
            if (isLoaded) {
                ...
                if (nativeAdView != null){
                    ...
                    if (view.callToActionView != null) {
                        view.callToActionView?.textSize = 18f
                        val ctaText = try {
                            cta?.lowercase()?.let {
                        ...
                        view.callToActionView?.text = ctaText ?: cta
                        view.callToActionView?.isAllCaps = false
                    }
