...
class AdmobAppOpenAdManager private constructor(private val application: Application) : CemOpenAd,
    ...
    private fun fetchAd(adUnitId: String?, adCallback: OpenLoadCallback? = null) {
        ...
        if (isAdAvailable()) {
            appOpenAd?.let {
                adCallback?.onAdLoaded(this@AdmobAppOpenAdManager)
            }
            return
        }
        ...
        loadCallback = object : AppOpenAdLoadCallback() {
            override fun onAdLoaded(p0: AppOpenAd) {
                ...
                p0.setOnPaidEventListener {
                    AppFlyersLogManager.logEventOpenAdsRevenue(p0, it)
                }
                loadTime = Date().time
                adCallback?.onAdLoaded(this@AdmobAppOpenAdManager)
            }
            ...
            override fun onAdFailedToLoad(p0: LoadAdError) {
                ...
                Log.d(TAG, "onAdFailedToLoad admob: $adUnitId ${p0.message}")
    ...
    private fun showAdIfAvailable() {
        ...
        if (isIgnoreActivity(currentActivity)) {
            Log.d(TAG, "showAdIfAvailable: $currentActivity")
            fullScreenContentCallback?.onAdFailedToShowCallback("currentActivity not show")
            Log.d(TAG, "showAdIfAvailable: currentActivity not show")
            return
        }
    ...
    private fun showAdIfAvailableDirect() {
        if (CemAdManager.getInstance(application).isVip()) {
            Log.d(TAG, "showAdIfAvailable: User vip")
            fullScreenContentCallback?.onAdFailedToShowCallback("user vip")
            return
    ...
    private fun isShowDirectFull(configKey: String?): Boolean {
        ...
        return ConfigManager.getInstance(application).adManagement?.placementList?.get(configKey)?.showDirect
            ?: false
    }
    ...
    override fun onActivityCreated(activity: Activity, savedInstanceState: Bundle?) {
        ...
        Log.d(TAG, "onActivityCreated: $currentActivity")
    }
    ...
    override fun onShowed(isShowDirect: Boolean, callback: InterstitialShowCallback?) {
    ...
    companion object {
        ...
        var INSTANCE: AdmobAppOpenAdManager? = null
        fun getInstance(application: Application): AdmobAppOpenAdManager {
            return INSTANCE ?: synchronized(this) {
                val instance = AdmobAppOpenAdManager(application)
                INSTANCE = instance
                instance
            }
        }
