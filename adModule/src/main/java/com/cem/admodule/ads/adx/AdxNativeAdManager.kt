...
import com.cem.admodule.ext.invisible
...
class AdxNativeAdManager @Inject constructor(
    ...
    override fun show(view: CustomNativeView) {
        try {
            view.setTemplateType(R.layout.admob_native_ad_view)
            if (isLoaded) {
    ...
    companion object {
        ...
        @JvmStatic
        fun newInstance(adUnit: String?): AdxNativeAdManager {
            return AdxNativeAdManager(adUnit)
        }
    }
}