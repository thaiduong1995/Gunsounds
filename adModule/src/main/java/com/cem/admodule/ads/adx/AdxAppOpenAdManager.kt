...
class AdxAppOpenAdManager private constructor(private val application: Application) : CemOpenAd,
    ...
    private fun showAdIfAvailableDirect() {
        if (CemAdManager.getInstance(application).isVip()) {
            Log.d(AdmobAppOpenAdManager.TAG, "showAdIfAvailable: User vip")
            fullScreenContentCallback?.onAdFailedToShowCallback("user vip")
            return
        }
        ...
        if (!isShowingAd && isAdAvailable()) {
            ...
            fullScreenContentCallback?.onAdFailedToShowCallback("không đủ điều kiện")
            Log.d(AdmobAppOpenAdManager.TAG, "showAdIfAvailable: $isShowingAd")
        }
    }
