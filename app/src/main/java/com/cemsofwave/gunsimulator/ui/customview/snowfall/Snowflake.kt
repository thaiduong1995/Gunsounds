/*
 * *
 *  * Created by duonglkh on 10/4/23, 4:59 PM
 *  * Copyright (c) 2023 . All rights reserved.
 *  * Last modified 10/4/23, 4:59 PM
 *
 */

package com.cemsofwave.gunsimulator.ui.customview.snowfall
...
internal class Snowflake(
    ...
    internal fun reset(positionY: Double? = null) {
        ...
        val angle =
            Math.toRadians(randomizer.randomDouble(params.angleMax) * randomizer.randomSignum())
        ...
        if (positionY > params.parentHeight) {
            if (shouldRecycleFalling) {
                if (stopped) {
                    ...
                    reset(positionY = -randomizer.randomDouble(params.parentHeight))
                }
            } else {
                positionY = -randomizer.randomDouble(params.parentHeight + size)
                stopped = true
            }
        }
    ...
    fun draw(canvas: Canvas) {
        ...
        if (localBitmap != null) {
            canvas.drawBitmap(localBitmap, positionX.toFloat(), positionY.toFloat()*-1, paint)
        } else {
            canvas.drawCircle(positionX.toFloat(), positionY.toFloat()*-1, size.toFloat(), paint)
        }
    }
